...
class CommentEmbedding(Layer):
    ...
    def create_variables(self):
        self.embedding_matrix = create_shared(self.vocabulary_size, self.hidden_size, name='CommentEmbedding.embedding_matrix')
        
    def activate(self, word_list):
        return self.embedding_matrix[word_list[:,0]]
...
class StackedCells(object):
    ...
    def __init__(self, input_size, celltype=RNN, layers=None,
                 activation=lambda x:x, down_direction=True):
        if layers is None:
            layers = []
        self.input_size = input_size
    ...
    def forward(self, x, prev_hiddens=None, time_hiddens=None, input_time=False, dropout=None):
        """
        Return new hidden activations for all stacked RNNs
        prev_hiddens and time_hiddens are defined by scan loop that calls this function
        """
        if dropout is None:
            dropout = []
        if prev_hiddens is None:
            prev_hiddens = [(T.repeat(T.shape_padleft(layer.initial_hidden_state),
                            ...
                            for layer in self.layers]
        '''
        if input_time and time_hiddens is None:
            time_hiddens = [(T.repeat(T.shape_padleft(layer.initial_hidden_state),
                                      x.shape[0], axis=0)
                             if x.ndim > 1 else layer.initial_hidden_state)
                            if hasattr(layer, 'initial_hidden_state') else None
                            for layer in self.layers]
        ...
        for k, layer in enumerate(self.layers):
            ...
            if layer.is_recursive:
                ...
                #else:
                #    level_out = layer.activate(level_out, prev_hiddens[k], time_hiddens[k])
            else:
